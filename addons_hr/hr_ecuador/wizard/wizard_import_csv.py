...
fields_form={
        ...
        'tipo' : {'string' : 'Tipo', 'type' : 'selection', 'selection' : [('pago','Pago Cooperativa'),
                                                                          ('food', 'Pago Alimentacion'),
                                                                          ('iess', 'Prestamos IESS'),
                                                                          ('ayuda','Egreso General')], 'required' : True},
        'data':{'string':'Archivo', 'type':'binary', 'required':True,},
        'otro' : {'string' : 'Otro Tipo?', 'type' : 'boolean'},
        'otro_name' : {'string' : 'Otro Tipo', 'type' : 'char'},
        }

class wizard_load_csv(osv.osv_memory):
    _name="wizard_load_csv"    
    ...
    def _load_csv(self, cr, uid, data, context):
        ...
        for item in buf:
            ...
            if em_id:
                if flt>0:
                    if otro == 1:
                        descripcion = form['otro_name']
                    else:
                        if tipo == 'pago':
                            descripcion = "Pago Cooperativa"
                        elif tipo == 'food':
                            descripcion = "Pago Comida"
                        elif tipo == "iess":
                            descripcion = "Prestamo IESS"
                        elif tipo == 'ayuda':
                            descripcion = "Egreso General"
